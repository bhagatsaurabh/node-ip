...
Terminal.prototype.setData = function (newData) {
  this.data = newData;
  //console.log('setData');
  if (this.connector != null && this.type == "out")
    this.pass(this.connector.terminalEnd);
...
var Thumbnail = function (height, onImageChange) {
  ...
  this.htmlInput.onchange = function (event) {
    ...
    this.parent.imageFileName = event.path[0].files[0].name.substring(
      0,
...
HorizontalLayout.prototype.draw = function () {
  if (Array.isArray(this.weights)) {
    var availableWidth =
      this.width - (this.components.length - 1) * this.spacing;
    var occupiedWidth = 0;
  ...
  if (this.placement == "left") {
    ...
    context.fillText(
      ...
      this.parent.x -
        this.parent.width / 2 +
        this.x -
        context.measureText(this.label).width / 2,
      this.parent.y - this.parent.height / 2 + this.y + globalFontSize / 3
    );
  } else {
    context.fillText(
      ...
      this.parent.x -
        ...
        this.x +
        this.width / 2 -
        context.measureText(this.label).width,
      this.parent.y - this.parent.height / 2 + this.y + globalFontSize / 3
    );
  }
};
...
ChoiceGroup.prototype.draw = function () {
  for (
    var choiceIndex = 0;
    choiceIndex < this.choiceNames.length;
    choiceIndex++
  ) {
...
Slider.prototype.slide = function (x) {
  if (x < this.parent.x - this.parent.width / 2 + this.x - this.width / 2) {
    ...
    this.value = Math.floor(
      ((x - (this.parent.x - this.parent.width / 2 + this.x - this.width / 2)) /
        (this.parent.x -
          this.parent.width / 2 +
          this.x +
          this.width / 2 -
...
BaseNode.prototype.draw = function () {
  ...
  context.roundUpperRect(
    this.x,
    this.y -
      this.height / 2 +
      globalOutlineWidth +
      (this.radius + globalBaseHeight) / 2,
    this.width - 4,
  ...
  context.moveTo(
    this.x - this.width / 2,
    this.y - this.height / 2 + this.radius + globalBaseHeight - 1
  );
  context.lineTo(
    this.x + this.width / 2,
    this.y - this.height / 2 + this.radius + globalBaseHeight - 1
