...
var renderWidth = -1, renderHeight = -1;

var renderingWait = document.getElementsByClassName('renderingWait')[0];
...
function renderOutput() {
    ...
    processor.onmessage = (e) => {
        ...
        if (outputNode.node.inputTerminals[0].data != null) {
            ...
            renderContext.drawImage(tempCanvas, (renderCanvas.width / 2) - (renderWidth / 2), (renderCanvas.height / 2) - (renderHeight / 2), renderWidth, renderHeight);
        }
    };
}

renderCanvas.addEventListener("mousewheel", function (event) {
        ...
        switch(currPairNode.renderNode.node.type) {
            ...
            case 'imagesource': {
                if (currPairNode.processNode.dataType == 'image')
                    ...
                    currPairNode.processNode.data = currPairNode.renderNode.node.outputTerminals[3].data;
                break;
            }
            case 'grayscale': {
...
function showRenderingWait() {
    renderingWait.style.visibility = 'visible';
    renderingWait.style.pointerEvents = 'all';
}

function hideRenderingWait() {
    renderingWait.style.visibility = 'hidden';
    renderingWait.style.pointerEvents = 'none';
}
