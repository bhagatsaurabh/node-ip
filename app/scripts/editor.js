...
editorCanvas.addEventListener(
  ...
  function () {
    ...
    switch (draggedItemText) {
      case "image": {
        width = editorWidth * 0.3 * globalScaleRatio;
        var newImageSourceNode = new ImageSourceNode(
          pos.x,
          pos.y,
          width,
          nodes.length,
      ...
      case "grayscale": {
        ...
        var newGrayscaleNode = new GrayscaleNode(
          pos.x,
          pos.y,
          width,
          nodes.length,
      ...
      case "binarize": {
        width = editorWidth * 0.3 * globalScaleRatio;
        var newBinarizeNode = new BinarizeNode(
          pos.x,
          pos.y,
          width,
          nodes.length,
      ...
      case "layer": {
        width = editorWidth * 0.3 * globalScaleRatio;
        var newLayerNode = new LayerNode(
          pos.x,
          pos.y,
          width,
          nodes.length,
      ...
      case "combinechannel": {
        width = editorWidth * 0.2 * globalScaleRatio;
        var newCombineChannel = new CombineChannelNode(
          pos.x,
          pos.y,
          width,
          nodes.length,
      ...
      case "brightness": {
        ...
        var newBrightness = new BrightnessNode(
          pos.x,
          pos.y,
          width,
          nodes.length,
      ...
      case "contrast": {
        width = editorWidth * 0.25 * globalScaleRatio;
        var newContrast = new ContrastNode(
          pos.x,
          pos.y,
          width,
          nodes.length,
      ...
      case "reducepalette": {
        ...
        var newReducePalette = new ReducePaletteNode(
          pos.x,
          pos.y,
          width,
          nodes.length,
...
function getNodeFromMousePos(pos) {
  ...
  var highestOrder = Math.max.apply(
    Math,
    inbounds.map(function (o) {
      return o.node.order;
    })
  );
  return inbounds.find(function (o) {
    return o.node.order == highestOrder;
  });
}

function pushOutputNode() {
  var width = editorWidth * 0.2 * globalScaleRatio;
  var newOutputNode = new OutputNode(
    editorCanvas.width / 2 - width / 2,
